...
def preprocess_input_image(image):
    """Apply OpenCV preprocessing before segmentation and recognition."""
    ...
    # Step 2: Brighten and increase contrast
    brightened = cv2.convertScaleAbs(gray, alpha=1.5, beta=30)

    # Step 3: Adaptive thresholding for black text on white background
...
def process_full_document(image, num_beams=3, progress=gr.Progress()):
    ...
    try:
        ...
        # Save uploaded image and preprocessed image
        original_image_path = os.path.join(temp_dir, 'input_image.jpg')
        image.save(original_image_path)

        preprocessed_gray = preprocess_input_image(image)
        image_path = os.path.join(temp_dir, 'preprocessed_input.jpg')
